...
for data_path in person:
    if 'healthy' in data_path:
        cls.append(0)
        fullname = glob.glob(
            '/home/cwx/extra/covid_project_segs/lungs/' + data_path.split('_')[0] + '/' + data_path + '_*.nii')+\
        glob.glob(
            '/mnt/data11/extra_segs/lungs/' + data_path.split('_')[0] + '/' + data_path + '_*.nii')
        try:
