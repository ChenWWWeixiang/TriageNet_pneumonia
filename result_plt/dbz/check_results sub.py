...
res=np.load('/mnt/data9/Lipreading-DenseNet3D-master/re/reader1_new.npylevel2.npy')
...
print(np.mean(np.argsort(-pre,-1)[:,1] == gt))
# for i in range(100):
#     train_x, test_x, train_y, test_y = train_test_split(pre, y_one_hot, test_size=0.00001)
...
answer='answer1.txt'
humans=['/mnt/data9/Lipreading-DenseNet3D-master/for_dbz_pre/1-reader record-5年.xlsx',
...
for id,onep in enumerate (humans):
...
#         gtmain=np.array(gtmain)
#         for i in range(16):
#             nspecifciity += np.sum((gtmain != i) * (predmain !=i).any()) 
#             nc+=np.sum(predmain != i)
#             sensitivity += np.sum((gtmain == i) * (predmain == i).any()) 
#             sc+=np.sum(predmain == i)
#        # ACC=pre==gt
#         nspecifciity=nspecifciity/nc
#         sensitivity=sensitivity/sc
#         plt.scatter(1-nspecifciity,sensitivity, c=color[7 + id - 1], marker=shape[7 + id - 1],s=10)
...
plt.title('Reader study I: Overall level-II ROC Curves')
plt.savefig('/mnt/data9/Lipreading-DenseNet3D-master/result_plt/dbz/reader1-sub.jpg',bbox_inches='tight')
