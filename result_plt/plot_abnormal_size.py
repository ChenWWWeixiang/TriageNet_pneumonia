...
import pandas as pd
all_files=os.listdir('../multi_period_scores/npys')
def ana_group(name,group):
    R=[]
    for item in group:
        data=np.load(os.path.join('../multi_period_scores/npys',item))
...
def show_bins(data,name,fig_name):
    ...
    for i in range(num_g):
        ...
        hist=hist/hist.sum()
        plt.bar(x+width/2+i*width,hist,width,alpha = 0.9,label=name[i]+' Mean={:.4f}'.format(np.mean(data[i])))
    plt.xlim([0,1])
    ...
    plt.show()
f=open('source_data.txt','w')
for item in all_files:
    data=np.load(os.path.join('../multi_period_scores/npys',item))
    ab_rate = np.mean(data > 0.5)
    f.writelines(item.split('_')[-1][0]+','+item.split('_')[-2]+','+item.split('_')[-3]+','+str(ab_rate)+'\n')
...
show_bins([r1,r2],['Male','Female'],'jpgs/ab_number_gender.jpg')

#age0=np.array(all_files)[age==0]
age4=np.array(all_files)[age==4]
age1=np.array(all_files)[age==1]
age2=np.array(all_files)[age==2]
age3=np.array(all_files)[age==3]
#r0=ana_group('<20',age0)
r1=ana_group('21-40',age1)
r2=ana_group('41-60',age2)
r3=ana_group('61-80',age3)
r4=ana_group('>81',age4)
show_bins([r1,r2,r3,r4],['21-40','41-60','61-80','>81'],'jpgs/ab_number_age.jpg')
STAGEI = []
STAGEII = []
...
show_bins([r1,r2],['stage1','stage2'],'jpgs/ab_number_stage.jpg')