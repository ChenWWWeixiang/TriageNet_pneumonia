...
with open(args.output_file,'w') as f:
    ...
    for a_res in ress:
        ...
        time = np.array(time)
        Data = np.concatenate([pre, gt[:,np.newaxis], sex[:,np.newaxis], age[:,np.newaxis],time[:,np.newaxis]],1)
...
Data=pd.DataFrame(Data,columns=['pre_nop','pre_cap','pre_flu','pre_covid','gt','sex','age','time'])
Data.to_csv('source_record.csv')
...
ax = plt.gca()
ax.legend_.remove()
plt.savefig('jpgs/bar_auc_2.jpg')
#plt.show()
