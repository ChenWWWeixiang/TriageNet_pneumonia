...
def get_CI(value,res,xx=False):
    ...
    if xx:
        ...
        res.append(str(round(np.mean(value),4)) + ' (' + str(round(confidence_lower,4)) + '-' + str(round(confidence_upper,4)) + ')')
    return res
import argparse
parser = argparse.ArgumentParser()

parser.add_argument("-i", "--ress", help="A list of npy files which record the performance.",
                    default=['../re/ind_try.npy'])
...
with open("record_3c.csv","w") as csvfile:
